...
import ReservationsRestaurateur from "./components/ReservationsRestaurateur";
...
const Dashboard = () => {
  ...
  useEffect(() => {
    async function componentDidMount() {
      ...
      const response = await fetch(`${serveur}/reservations/restaurant`, {
        method: 'GET',
        headers: {
      ...
      <main>
        <Container maxWidth="md">
          <div className={classes.header}>
            <Typography className={classes.title} variant="h2" gutterBottom> Tableau de bord </Typography>
          </div>
          <div className='reservations'>
            <div>
              <div className={classes.header}>
                <Typography variant="h4" gutterBottom> Les réservations: </Typography>
              </div>
              <div className={classes.reservationContent}>
                { reservation !== null &&
                  <Grid container spacing={4}>
                    {reservation.map((res) => (
                      <ReservationsRestaurateur reservation={res} key={res.id} idResto={res.restaurantId}/>
                    ))}
                  </Grid>
                }
